...
async function cargarSecuencial() {
  ...
  try {
    ...
    mostrarLog(`✅ Secuencial completado en ${formatearTiempo(tiempoSecuencial)}`, 'success');
    mostrarComparativa();
  } catch (error) {
...
async function cargarParalelo() {
  ...
  try {
    ...
    mostrarLog(`✅ Paralelo completado en ${formatearTiempo(tiempoParalelo)}`, 'success');
    mostrarComparativa();
  } catch (error) {
    mostrarLog(`❌ Error: ${error.message}`, 'error');
  }
...
async function fetchConReintentos(nombre, intentos = 3) {
  ...
  for (let i = 0; i < intentos; i++) {
    try {
      ...
      // 50% de probabilidad de fallo para ver los reintentos
      ...
      if (i < intentos - 1) {
        const espera = Math.pow(2, i) * 500; // Backoff exponencial: 500, 1000, 2000ms
